...
if (_indexAzioni != -1) then {
	if ((ListaAzioni select _indexAzioni) select 2) then {
		if (_indexGiocatori != -1) then {
			...
			if !(((profileNamespace getVariable "XEATV_Other") select 1) == "DAH") then {
				if !(((profileNamespace getVariable "XEATV_Other") select 1) == "DAH") then {
					hint parseText format [localize"STR_XEAT_EsecuzioneErrorNoGiocatore", XeHintHeader, format["<t color='#f4e242'>%1</t>",(ListaAzioni select _indexAzioni) select 0]];
				};
			};
		};
	} else {
	...
	if !(((profileNamespace getVariable "XEATV_Other") select 1) == "DAH") then {
		if !(((profileNamespace getVariable "XEATV_Other") select 1) == "DAH") then {
			hint parseText format [localize"STR_XEAT_EsecuzioneErrorNoAzione", XeHintHeader];
		};
	};
};